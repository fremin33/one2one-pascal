<?php
	...
	if(isset($_GET["numCarte"]) && isset($_GET["id"]) && !empty($_GET["numCarte"]) && !empty($_GET["id"]))
	{
		...
		if(!empty($carteExiste))
		{
			...
			if(!empty($resa))
			{
				...
				//1 heure avant remplacer par 24h
				$dateJourPlus24=date('Y-m-d H:i:s',strtotime('+0 hour',strtotime($dateJour)));
	...
	$tabRep=array("texte"=>$reponse,"valPlace"=>$valPlace,"adresseMail"=>$mail,"contenuMail"=>$contenuMail);
