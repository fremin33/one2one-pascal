<?php
	...
	if(!empty($carteExiste))
	{
		...
		if(empty($resa))
		{
								...
								{
									if($v['idFormule']==5 || $v['idFormule']==6 || $v['idFormule']==10)    /* FIT GROUP OU CARTE */
									{
							...
							if($enregistrement!=-1)
							{
								...
								//$dateFinAbonnementPlus4mois=date('Y-m-d',strtotime('+4 month',strtotime($dateFinAbonnement)));
...
$nomDuCours=$lesCours[0]['nom'];		
											...
											if($placeRestante[0]['nombrePlaces']>0)
											{
												...
												$reponse.="Votre réservation a été effectuée pour le cours du ".$placeRestante[0]['date']." à ".$placeRestante[0]['heure'];
